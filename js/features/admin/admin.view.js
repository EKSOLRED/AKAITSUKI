...
function renderAboutSocialRows(socials = [], escapeHtml) {
  ...
  return socials.map((social, index) => `
    <div class="about-social-admin-row glass-soft" data-about-social-row data-social-id="${social.id}" draggable="true">
      ...
      <div class="form-grid form-grid--2 compact-grid">
        <input class="input" data-about-social-field="icon" data-social-id="${social.id}" value="${escapeHtml(social.icon || '')}" placeholder="Иконка: локальный файл или URL" />
    ...
    <section class="page" data-page="admin">
      <section class="section-card glass admin-layout-fixed">
        <div class="section-header">
          ...
          <div class="admin-toolbar">${ctx.isSuperAdmin() && !ctx.isSupabaseData ? `<button class="button button--ghost" id="resetDbButton">Сбросить demo-данные</button>` : ``}</div>
        </div>
