...
import { escapeHtml as escapeHtmlUtil, sanitizeHref, sanitizeMediaSrc } from './utils/text.utils.js';
...
const authModal = setupAuthModal({
  onSuccess: async () => {
    try {
      ...
      await getDataAdapter().init?.(true);
    } catch (error) {
      console.error('Post-auth init error', error);
    }
    rerender();
  },
});
...
function renderUserBox() {
  ...
  document.getElementById('logoutButton')?.addEventListener('click', async () => {
    try {
      ...
      if (route === '/favorites' || route === '/admin') {
        location.hash = '#/catalog';
      } else {
        rerender();
      }

      getDataAdapter().init?.(true).catch((error) => {
        console.error('Post-logout data refresh error', error);
      });
    ...
    <div class="site-footer__inner">
      ...
      <div class="site-footer__socials">
        ${socials.map((social) => `
          ...
          >
            ${social.icon
              ? `<img class="site-footer__social-icon" src="${sanitizeMediaSrc(social.icon)}" alt="" />`
...
function createSharedContext() {
  return {
    ...
    isSupabaseData: getDataAdapter().mode === 'supabase',
    showToast,
    escapeHtml,
    createFavoriteButton,
    createSelectControl,
